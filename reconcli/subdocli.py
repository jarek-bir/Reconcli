...
@click.command()
@click.option('--domain', '-d', required=True, help='Target domain for subdomain enumeration')
@click.option('--output-dir', '-o', default='output', help='Directory to save results')
@click.option('--amass-config', default=os.path.expanduser('~/.config/amass/config.ini'), help='Path to Amass config')
...
@click.option('--timeout', default=30, help='Timeout for individual operations (seconds)')
@click.option('--threads', default=50, help='Number of threads for concurrent operations')
@click.option('--all-tools', is_flag=True, help='Use all available tools (including active)')
...
def subdocli(domain, output_dir, amass_config, markdown, resolve, probe_http, verbose, 
            timeout, threads, all_tools, active, resume, clear_resume, show_stats):
    ...
    http_results = []
    if probe_http:
        targets = resolved_subs if resolved_subs else [{"subdomain": sub} for sub in all_subs]
        if verbose:
...
def resolve_subdomains(subdomains, threads, verbose):
    ...
    def resolve_single(subdomain):
        try:
            ...
            return {"subdomain": subdomain, "ip": None, "resolved": False, "error": str(e)}
    
    with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
        future_to_subdomain = {executor.submit(resolve_single, sub): sub for sub in subdomains}
    ...
    if verbose:
        click.echo(f"[+] ✅ Successfully resolved {len(successful)}/{len(subdomains)} subdomains")
...
def probe_http_services(targets, timeout, threads, verbose):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
        future_to_target = {executor.submit(probe_single, target): target for target in targets}
    ...
    accessible = [r for r in results if r["http"]["accessible"] or r["https"]["accessible"]]
    if verbose:
...
def generate_enhanced_markdown_report(outpath, domain, data, verbose):
    ...
    with open(md_path, 'w') as f:
        ...
        f.write("|------|------------------|\n")
        for tool, count in sorted(data['tool_stats'].items(), key=lambda x: x[1], reverse=True):
        ...
        if data['resolved']:
            ...
            f.write(f"**Successfully Resolved:** {resolved_count}/{len(data['resolved'])}\n\n")
        ...
        if data['http_services']:
            http_accessible = [r for r in data['http_services'] if r['http']['accessible']]
            https_accessible = [r for r in data['http_services'] if r['https']['accessible']]
...
def display_scan_statistics(data, tool_stats):
    ...
    if data['http_services']:
        ...
        https_count = len([r for r in data['http_services'] if r['https']['accessible']])
        click.echo(f"\n[+] 🌐 HTTP Services:")
