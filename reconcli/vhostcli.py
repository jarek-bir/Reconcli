...
def cli(
    ...
    if port_scan:
        click.echo(f"\n🔍 Running port discovery with {port_scanner}...")
        ...
        if verbose:
            ...
            click.echo(
                f"📊 Port discovery found {total_ports} open ports across {len(targets)} targets"
            )
    ...
    for target_idx, target_ip in enumerate(targets, 1):
        ...
        if nuclei_scan and results:
            if verbose:
                click.echo("🧪 Running nuclei vulnerability scan...")
            ...
            if nuclei_results:
                ...
                click.echo(
                    f"🧪 Found {len(nuclei_results)} vulnerabilities with nuclei"
                )
...
def run_port_discovery(targets, scanner, verbose):
    ...
    for target_ip in targets:
        if verbose:
            click.echo(f"🔍 Scanning {target_ip} for web services...")
        ...
        try:
            ...
            if verbose:
                click.echo(f"⚠️ Port scan failed for {target_ip}: {e}")

        if open_ports:
            discovered_ports[target_ip] = open_ports
            if verbose:
                click.echo(
                    f"✅ Found {len(open_ports)} open web ports on {target_ip}: {open_ports}"
                )
        else:
            if verbose:
                click.echo(f"❌ No web ports found on {target_ip}")
...
def run_nuclei_scan(vhosts, templates, severity, verbose):
    ...
    if not shutil.which("nuclei"):
        click.echo(
            "❌ nuclei not found. Install with: go install -v github.com/projectdiscovery/nuclei/v3/cmd/nuclei@latest"
        )
        return []
    ...
    try:
        ...
        if verbose:
            click.echo(f"🚀 Running nuclei: {' '.join(cmd)}")
        ...
        if verbose:
            click.echo(f"🧪 Nuclei found {len(nuclei_results)} vulnerabilities")

    except (subprocess.TimeoutExpired, subprocess.CalledProcessError) as e:
        if verbose:
            click.echo(f"⚠️ Nuclei scan failed: {e}")
    finally:
        # Cleanup
